...
public static class callback
{
	...
	public static string[] mutedClients;
	...
	*/
	public static void onNewChannelEvent(uint64 serverConnectionHandlerID, uint64 channelID, uint64 channelParentID)
	{
		...
		error = ts3client.getChannelVariableAsString(serverConnectionHandlerID, channelID, ChannelProperties.CHANNEL_NAME, out namePtr);
	...
	*/
	public static void onNewChannelCreatedEvent(uint64 serverConnectionHandlerID, uint64 channelID, uint64 channelParentID, anyID invokerID, string invokerName, string invokerUniqueIdentifier)
	{
		...
		uint error = ts3client.getChannelVariableAsString(serverConnectionHandlerID, channelID, ChannelProperties.CHANNEL_NAME, out namePtr);
		if (error != public_errors.ERROR_ok)
		{
			return;
		}
	...
	*/
	public static void onClientMoveSubscriptionEvent(uint64 serverConnectionHandlerID, anyID clientID, uint64 oldChannelID, uint64 newChannelID, int visibility)
	{
		...
		uint error = ts3client.getClientVariableAsString(serverConnectionHandlerID, clientID, ClientProperties.CLIENT_NICKNAME, out namePtr);
		if (error != public_errors.ERROR_ok)
		{
			return;
		}
		...
		for (int i = 0; i < mutedClients.Length; i++)
		{
			if (mutedClients[i].Equals(name))
			{
				anyID[] ids = { clientID, 0 };
				IntPtr result;
				ts3client.requestMuteClients(serverConnectionHandlerID, ids, out result);
			}
		}
	}
	...
	*/
	...
	/// <summary>
	/// Userlogging-Callback. Wrappt das Unity-Logging
	/// </summary>
	...
	/// <param name="completeLogString"></param>
	...
	{
		string message = "TEAMSPEAK: " + logTime + " | " + logID + " | " + logChannel + " | " + logmessage;
		if (logLevel > (int)LogLevel.LogLevel_WARNING)
		{
			Logger.Log(message);
		}
		else if (logLevel > (int)LogLevel.LogLevel_ERROR)
		{
